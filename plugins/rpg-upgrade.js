...
untuk mendapatkannya ketik *${usedPrefix}craft fishingrod*`
                ...
                conn.send2Button(m.chat, `Succes mengupgrade *${rpg.emoticon('fishingrod')}FishingRod*`, m, 'menu', usedPrefix + 'menu', 'inventory', usedPrefix + 'inv', m)
                break
            case 'pickaxe':
                if (pickaxe == 0) {
                    const buttons = [
...
untuk memilikinya ketik *${usedPrefix}craft Pickaxe*`
                ...
                if (pickaxe > 9) return conn.sendButton(m.chat, `*${rpg.emoticon('pickaxe')}Pickaxe* kamu sudah level max!!`, m, 'inventory', usedPrefix + 'inv', m)
...
${batu < __batu ? `\n${rpg.emoticon('batu')}Batu kamu kurang *${__batu - batu}*` : ''}${kayu < __kayu ? `\n${rpg.emoticon('kayu')}Kayu kamu kurang *${__kayu - kayu}*` : ''}${money < __money ? `\n${rpg.emoticon('money')}Uang kamu kurang *${__money - money}*` : ''}`, m, 'cek inventory', usedPrefix + 'inv', m)
                ...
                conn.sendButton(m.chat, `Succes mengupgrade *${rpg.emoticon('pickaxe')}Pickaxe*`, m, 'inventory', usedPrefix + 'inv', m)
                break
            case 'sword':
                if (sword == 0) {
                    const buttons = [
...
${iron < _iron ? `\n${rpg.emoticon('iron')}Iron kamu kurang *${_iron - iron}*` : ''}${kayu < ___kayu ? `\n${rpg.emoticon('kayu')}Kayu kamu kurang *${___kayu - kayu}*` : ''}${money < ___money ? `\n${rpg.emoticon('money')}Uang kamu kurang *${___money - money}*` : ''}`, m, 'inventory', usedPrefix + 'inv', m)
                ...
                conn.sendButton(m.chat, `Succes mengupgrade *${rpg.emoticon('sword')}Sword*`, m, 'inventory', usedPrefix + 'inv', m)
                break
            default :
