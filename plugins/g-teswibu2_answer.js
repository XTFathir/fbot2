...
${isSurrender ? '' : `+${room.winScore} XP tiap jawaban benar`}
    `.trim()
    const msg = await this.sendButton(m.chat, caption, author, null, [[`${(isWin || isSurrender) ? 'Family 100' : 'Nyerah'}`, `${(isWin || isSurrender) ? '.teswibu2' : 'nyerah'}`]], null, {
